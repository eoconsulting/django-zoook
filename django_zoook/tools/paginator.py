...
from django_zoook.config import PAGINATION_DEFAULT_TOTAL, PAGINATOR_DEFAULT_MODE
...
def set_paginator_options(request, default = 'position'):
    ...
    if 'mode' in request.session:
        request.session['mode'] = request.GET.get('mode') and request.GET.get('mode') or request.session['mode'] or PAGINATOR_DEFAULT_MODE
    else:
        request.session['mode'] = request.GET.get('mode') and request.GET.get('mode') or PAGINATOR_DEFAULT_MODE
